# Sosol-specific configuration for tomcat6

class sosol::tomcat($max_memory, $min_memory, $heap, $admin_pwd) {

  tomcat::instance { 'sosol':
  ...
  # we need to be sure tomcat running sosol can read these
  ...
  file { '/etc/ssl/certs/perseidsshibcert.pem':
    owner   => 'tomcat6',
    group   => 'tomcat6',
    content => lookup('sosol::shib::perseids_cer'),
  ...
  file { '/etc/ssl/private/private.key':
    owner   => 'tomcat6',
    group   => 'tomcat6',
    content => lookup('sosol::shib::key'),
    mode    => '0640',
    require => File['/etc/ssl/private'],
  }

  file { '/etc/ssl/certs/tuftsprod.crt':
    owner   => 'tomcat6',
    group   => 'tomcat6',
    content => lookup('sosol::shib::tufts_cer'),
    require => File['/etc/ssl/private'],
  }

  file { '/etc/ssl/certs/aai.crt':
    owner   => 'tomcat6',
    group   => 'tomcat6',
    content => lookup('sosol::shib::aai_cer'),
    require => File['/etc/ssl/private'],
  }
